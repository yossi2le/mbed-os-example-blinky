##############################
Test Case API for Mbed-clitest
##############################
...
`Mbed-clitest` command line parameters, or by the test suite.

Each test case can also contain a setup function and a teardown function
(rampUp and rampDown or setUp and tearDown in older test cases).
If setup fails, the case function is skipped and teardown is called.
...
1. "duts"
    * "*", dictionary, contains default requirements for all nodes
        ...
        * "type", type of duts,
          allowed values: hardware(default), process, simulate, serial, mbed
        ...
        * "platform_map", dictionary of platform names with dictionaries containing platform
        specific values as values. These can be used by Allocators (for example) Currently used
        values are:
            * "bin", binary for this platform. Value of bin under "application" (see below) will
        ...
        * "application", dictionary of application details
            ...
            * "bin", binary for dut/duts, path as string. This is either flashed onto the board or
            started as a process (if type is process).
            ...
            * "sync_cli", dictionary for configuring cli synchronization.
                * "retries", int, maximum amount of synchronization commands sent before synch
                fails. Default is 5 commands.
                * "command_timeout", int, timeout of a single synchronization command. Default is
                 5 seconds.
            * "cli_ready_trigger", string with prefix "regex:"
              or no prefix. If this is defined, Mbed-clitest will
...
`bench.py <mbed_clitest/bench.py>`_.
...
fails.
Parameters are dut nick, index or "*", generator function that must return a tuple, list of
arguments for generator function, amount of attempts to be made to synchronize, timeout of each
attempt.
...
from/mbed_clitest.TestStepError module.
...
By default, the internally behaviour for releasing resources will reset the device.
However, you do need to specify the behaviour you want.
Please note, this function is only for RAAS right now.
...
are implemented in `asserts.py <../mbed_clitest/tools/asserts.py>`_.
...
Mbed-clitest contains a decorator called test_case which can be used
...
**NOTE**: If the object that is read from a test case configuration file contains duplicate keys, Mbed-clitest will fail and exit.
...
keys, Mbed-clitest will fail and exit.
